...
import { CalendarIcon, Link, Plus, Trash, Loader2 } from "lucide-react"
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover"
import { format } from "date-fns"
import { Calendar } from "@/components/ui/calendar"
import { createInterview, getUsers, type InterviewCreateData } from "@/lib/api/interviews"
...
export default function InterviewScheduler({
  ...
  const [interviewDetails, setInterviewDetails] = useState({
    ...
    time: "",
    duration: "60", // minutes
    interviewers: [{ id: 1, userId: null as number | null }],
    notes: "",
  ...
  // Load available users for interviewer selection
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        toast({
          title: "Error",
          description: "Failed to load available interviewers. Please refresh the page.",
          variant: "destructive"
        })
      } finally {
        setIsLoading(false)
  ...
  const handleInterviewerChange = (id: number, userId: number | null) => {
    setInterviewDetails((prev) => ({
      ...prev,
      interviewers: prev.interviewers.map((interviewer) =>
        interviewer.id === id ? { ...interviewer, userId } : interviewer,
      ),
    }))
  }
  ...
  const addInterviewer = () => {
    setInterviewDetails((prev) => ({
      ...
      interviewers: [
        ...prev.interviewers,
        { id: prev.interviewers.length ? Math.max(...prev.interviewers.map((i) => i.id)) + 1 : 1, userId: null },
      ],
    }))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!interviewDetails.type || !interviewDetails.time) {
      toast({
        title: "Validation Error",
        description: "Please fill in all required fields (Type, Time).",
        variant: "destructive"
      })
      return
    }
    
    const selectedInterviewers = interviewDetails.interviewers
      .filter(i => i.userId !== null)
      .map(i => i.userId as number)
    ...
    try {
      ...
      const [hours, minutes] = interviewDetails.time.split(':')
      scheduledDateTime.setHours(parseInt(hours), parseInt(minutes))
      
      const interviewData: InterviewCreateData = {
        application: applicationId,
        interviewers: selectedInterviewers,
        lead_interviewer: selectedInterviewers[0], // First interviewer as lead
        interview_type: interviewDetails.type as InterviewCreateData['interview_type'],
        scheduled_at: scheduledDateTime.toISOString(),
        duration_minutes: parseInt(interviewDetails.duration),
        internal_notes: interviewDetails.notes || undefined,
        send_calendar_invite: interviewDetails.sendCalendarInvite,
        send_confirmation_email: interviewDetails.sendConfirmationEmail,
      }
      ...
      toast({
        title: "Success!",
        description: `Interview has been scheduled for ${candidateName}.`,
        variant: "default"
      })
            ...
            {interviewDetails.interviewers.map((interviewer, index) => (
              <div key={interviewer.id} className="flex items-center gap-2">
                <Select 
                  ...
                  onValueChange={(val) => handleInterviewerChange(interviewer.id, val ? parseInt(val) : null)}
                >
                  ...
                  <SelectContent>
                    {isLoading ? (
                      <SelectItem value="" disabled>Loading interviewers...</SelectItem>
                    ) : (
